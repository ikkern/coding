...
def draw_interface():
    ...
    up = center_font.render("MENU", True, BLACK)
    screen.blit(up, (WIDTH // 2 - up.get_width() // 2, 360))

    down = center_font.render("⏯", True, BLACK)
    screen.blit(down, (WIDTH // 2 - down.get_width() // 2, 520))

    left = center_font.render("<<", True, BLACK)
    screen.blit(left, (140, 445))

    right = center_font.render(">>", True, BLACK)
    screen.blit(right, (WIDTH - 160, 445))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                ...
                # Here you could change to another screen or play music, etc.
